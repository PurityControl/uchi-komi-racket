...
(define (grid-seq-products grid seq-len)
  (map (lambda (x) (posn-products x seq-len grid)) (grid-posns grid)))

(define (grid-seq-max-product grid seq-len)
  (apply max
        (map (lambda (x) (apply max x)) (grid-seq-products grid seq-len))))
...
(define (posn-value pos grid)
  (vector-ref (grid-values grid) (+ (* (grid-length grid) (sub1 (posn-y pos))) (sub1 (posn-x pos)))))

(define (posn-seqs pos seq-len grid)
  "a list of all possible sequences at position pos in grid grid with sequence length seq-len"
  (list
...
(define (posn-products pos seq-len grid)
  ...
  (lambda (pos len)
    (define (collect-seq lo-posns)
      (if (= len (length lo-posns))
          lo-posns
          (collect-seq (cons (posn-at-delta (car lo-posns) dx dy) lo-posns))))
    (collect-seq (list pos))))

(define (horizontal-seqs pos length)
  ((contigous-seqs 1 0) pos length))

(define (vertical-seqs posn length)
  ((contigous-seqs 0 1) posn length))
...
(define file-tests
  (test-suite
   ...
   (check-true ((horizontal? 3) (grid-factory 3 "1 2 3 4 5 6") (posn 1 1)))
   (check-false ((horizontal? 3) (grid-factory 3 "1 2 3 4 5 6") (posn 2 1)))
   ...
   (check-equal? (posn-value (posn 2 1) (grid-factory 2 "1 2 3 4 5 6")) 2)
   (check-equal? (posn-value (posn 1 2) (grid-factory 2 "1 2 3 4 5 6")) 3)
   (check-equal? (posn-value (posn 2 2) (grid-factory 2 "1 2 3 4 5 6")) 4)
   (check-equal? (posn-value (posn 1 3) (grid-factory 2 "1 2 3 4 5 6")) 5)
   (check-equal? (posn-value (posn 2 3) (grid-factory 2 "1 2 3 4 5 6")) 6)
   (check-equal? (apply +
   ...
   (check-equal? (posn-products (posn 1 1) 2 (grid-factory 2 "1 2 3 4 5 6")) '(2 3 4 1))
   (check-equal? (grid-seq-max-product (grid-factory 20 euler-grid) 4) 70600674)))
